...
compared2best  = dict({'RESTAURANT#GENERAL':2, 'FOOD#QUALITY':4, 'RESTAURANT#MISCELLANEOUS':6,  'FOOD#PRICES':7, 'DRINKS#QUALITY':7, 'LOCATION#GENERAL':3, 'RESTAURANT#PRICES':4, 'AMBIENCE#GENERAL':5, 'DRINKS#STYLE_OPTIONS':9, 'SERVICE#GENERAL':1, 'DRINKS#PRICES':8 , 'FOOD#STYLE_OPTIONS':4});
compared2worst  = dict({'RESTAURANT#GENERAL':8, 'FOOD#QUALITY':6, 'RESTAURANT#MISCELLANEOUS':4,  'FOOD#PRICES':3, 'DRINKS#QUALITY':3, 'LOCATION#GENERAL':7, 'RESTAURANT#PRICES':6, 'AMBIENCE#GENERAL':5, 'DRINKS#STYLE_OPTIONS':1, 'SERVICE#GENERAL':9, 'DRINKS#PRICES':2 , 'FOOD#STYLE_OPTIONS':6});
...
writer = pd.ExcelWriter('Ranks4.xlsx', engine='xlsxwriter')
df.to_excel(writer, sheet_name='Ranks', index=False)
writer.save()
                
df = pd.DataFrame(hotel_Scores)
writer = pd.ExcelWriter('Scores4.xlsx', engine='xlsxwriter')
df.to_excel(writer, sheet_name='Scores', index=False)
writer.save()  

df = pd.DataFrame(U_Name)
writer = pd.ExcelWriter('Hnames4.xlsx', engine='xlsxwriter')
df.to_excel(writer, sheet_name='Names', index=False)
writer.save()

 ############# BoxPlot #######################

df = pd.read_excel('Scores4.xlsx')
sns.boxplot(x=df)
