...
compared2best  = dict({'RESTAURANT#GENERAL':1, 'FOOD#QUALITY':1, 'RESTAURANT#MISCELLANEOUS':8,  'FOOD#PRICES':3, 'DRINKS#QUALITY':4, 'LOCATION#GENERAL':6, 'RESTAURANT#PRICES':2, 'AMBIENCE#GENERAL':9, 'DRINKS#STYLE_OPTIONS':6, 'SERVICE#GENERAL':5, 'DRINKS#PRICES':4 , 'FOOD#STYLE_OPTIONS':5});
compared2worst  = dict({'RESTAURANT#GENERAL':9, 'FOOD#QUALITY':9, 'RESTAURANT#MISCELLANEOUS':2,  'FOOD#PRICES':7, 'DRINKS#QUALITY':6, 'LOCATION#GENERAL':4, 'RESTAURANT#PRICES':8, 'AMBIENCE#GENERAL':1, 'DRINKS#STYLE_OPTIONS':4, 'SERVICE#GENERAL':5, 'DRINKS#PRICES':6 , 'FOOD#STYLE_OPTIONS':5});
...
writer = pd.ExcelWriter('Ranks3.xlsx', engine='xlsxwriter')
df.to_excel(writer, sheet_name='Ranks', index=False)
writer.save()
                
df = pd.DataFrame(hotel_Scores)
writer = pd.ExcelWriter('Scores3.xlsx', engine='xlsxwriter')
df.to_excel(writer, sheet_name='Scores', index=False)
writer.save()  


df = pd.DataFrame(U_Name)
writer = pd.ExcelWriter('Hnames3.xlsx', engine='xlsxwriter')
df.to_excel(writer, sheet_name='Names', index=False)
writer.save()

 ############# BoxPlot #######################

df = pd.read_excel('Scores3.xlsx')
sns.boxplot(x=df)               
